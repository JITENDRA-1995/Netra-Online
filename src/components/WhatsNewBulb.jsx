...
export function WhatsNewBulb({ isClientPortal = false }) {
  const [showBulb, setShowBulb] = useState(() => {
    return localStorage.getItem(`netra_whatsnew_v253_${isClientPortal ? 'client' : 'admin'}`) !== 'true';
  });
  ...
  const handleGotIt = () => {
    localStorage.setItem(`netra_whatsnew_v253_${isClientPortal ? 'client' : 'admin'}`, 'true');
    setShowBulb(false);
    setIsOpen(false);
  };
            ...
            >
              ...
              <div className="p-6 border-b border-white/5 bg-amber-500/5 flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-amber-400/80 uppercase tracking-widest mt-1">Netra OS v2.5.3 Update</p>
                </div>
              ...
              <div className="p-6 space-y-4">
                <p className="text-sm text-white/80 leading-relaxed mb-4">
                  We've enabled Progressive Web App (PWA) support! You can now run Netra OS as a standalone desktop or mobile application.
                </p>
                ...
                <ul className="space-y-3">
                  ...
                  <li className="flex gap-3 items-start">
                    <CheckCircle2 className="w-5 h-5 text-cyan-400 shrink-0 mt-0.5" />
                    <div>
                      <h4 className="text-sm font-bold text-white">Real-Time Sync Safety</h4>
                      <p className="text-xs text-muted-foreground mt-1 leading-relaxed">All Supabase real-time database queries bypass caching layers entirely, guaranteeing data is always fresh and accurate.</p>
                    </div>
                  </li>
                </ul>
              </div>
