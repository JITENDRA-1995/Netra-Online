...
export function WhatsNewBulb({ isClientPortal = false }) {
  const [showBulb, setShowBulb] = useState(() => {
    return localStorage.getItem(`netra_whatsnew_v256_${isClientPortal ? 'client' : 'admin'}`) !== 'true';
  });
  ...
  const handleGotIt = () => {
    localStorage.setItem(`netra_whatsnew_v256_${isClientPortal ? 'client' : 'admin'}`, 'true');
    setShowBulb(false);
    setIsOpen(false);
  };
            ...
            >
              ...
              <div className="p-6 border-b border-white/5 bg-amber-500/5 flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-amber-400/80 uppercase tracking-widest mt-1">Netra OS v2.5.6 Update</p>
                </div>
              ...
              <div className="p-6 space-y-4">
                <p className="text-sm text-white/80 leading-relaxed mb-4">
                  We've rolled out Netra OS v2.5.6 with stability fixes, Progressive Web App (PWA) support, and a fully redesigned alerts interface!
                </p>
