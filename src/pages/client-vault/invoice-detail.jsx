...
export function ClientInvoiceDetail({ invoiceId, onTabChange }) {
  ...
  return (
    <div className="space-y-8 max-w-4xl mx-auto animate-in fade-in duration-500 client-vault-theme">
      ...
      <div className="w-full overflow-x-auto pb-4 scrollbar-thin scrollbar-thumb-border scrollbar-track-transparent flex flex-col gap-8">
        {pages.map((pageItems, pageIdx) => {
          const isLastPage = pageIdx === pages.length - 1;
          const blankRowsCount = Math.max(0, rowsPerPage - pageItems.length);
          const pageSubtotal = pageItems.reduce((sum, item) => sum + (parseFloat(item.unitPrice) * (item.quantity || 1)), 0);
                  ...
                  <div style={{ width: '80px', height: '80px', background: '#fff', padding: '4px', borderRadius: '4px', border: '1px solid #ddd', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                    <img src={`https://api.qrserver.com/v1/create-qr-code/?size=150x150&data=${encodeURIComponent(
                      `upi://pay?pa=${bankingDetails.upiId}&pn=${bankingDetails.accountName}&am=${
                        isLastPage ? (isCompleted ? grandTotal : (advance > 0 ? remaining : grandTotal)) : pageTotal
                      }&cu=INR`
